...
def search(keywords, start_page = 1, end_page = 1, wangpan = WangPan.All):
	...
	print search_url
        ...
        #r = requests.get(search_url.decode('utf-8').encode('gbk'))
	#print type(search_url)
        ...
        if len(result_list) == 0:
            print "*********************************************"
            print "对不起，这个真没有！".decode('utf-8').encode('gbk')
            print "虽然已经很努力了，但还是没能找到您想要的结果。".decode('utf-8').encode('gbk')
            print "*********************************************", '\n'
...
while True:
    try:
        page = int(raw_input("请输入搜索结果的显示页数（每页10条结果）:".decode('utf-8').encode('gbk')))
    except Exception:
        print "输入有误，必须输入一个大于0的整数.".decode('utf-8').encode('gbk')
        continue
    else:
        if page <= 0:
            print "输入有误，必须输入一个大于0的整数.".decode('utf-8').encode('gbk')
